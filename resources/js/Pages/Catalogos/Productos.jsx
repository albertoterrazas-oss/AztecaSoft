        ...
        <div className="bg-white rounded-[2rem] p-3 shadow-sm border-2 border-transparent hover:border-slate-200 hover:shadow-xl transition-all duration-300 group">
            ...
            <div className="px-2 py-4">
                ...
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-4">
                    {product.UnidadMedida} • {product.Clasificacion || 'GENERAL'}
                </p>
...
export default function Productos() {
    ...
    const fetchData = async () => {
        try {
            setIsLoading(true);
            const [resProds, resAlmacenes] = await Promise.all([
                fetch("/api/productos").then(r => r.json()),
                axios.get(route("AlmacenesRefrigerados")) 
            ]);
    ...
    const handleOpenModal = (item = null) => {
        if (item) {
            ...
            setFormData({ 
                ...item, 
                ProductoPadre: item.ProductoPadre || "",
                Clasificacion: item.Clasificacion || "CARNE",
                // Mapeamos los IDs desde el objeto "almacenes" que manda tu API
                AlmacenesSeleccionados: item.almacenes ? item.almacenes.map(a => a.IdAlmacen) : []
            });
        } else {
            setAction('create');
    ...
    const toggleAlmacen = (id) => {
        setFormData(prev => ({
            ...prev,
            AlmacenesSeleccionados: prev.AlmacenesSeleccionados.includes(id)
                ? prev.AlmacenesSeleccionados.filter(item => item !== id)
                : [...prev.AlmacenesSeleccionados, id]
        }));
    };
    ...
    const handleSave = async (e) => {
        ...
        const isEdit = action === 'edit';
        try {
            await request(isEdit ? `/api/productos/${formData.IdProducto}` : "/api/productos", "POST", { 
                ...formData, 
            ...
            // REFRESCAMOS LA PETICIÓN DE PRODUCTOS
            await fetchData(); 
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] p-6 md:p-12 font-sans">
            <div className="max-w-[1600px] mx-auto">
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
                    <div>
                        <h1 className="text-4xl font-black text-[#1B2654] tracking-tighter uppercase leading-none">Productos</h1>
                        {/* <p className="text-[10px] text-slate-400 font-black tracking-[0.4em] uppercase mt-3">SIAF4 - Almacenamiento & Logística</p> */}
                    </div>
                    <button onClick={() => handleOpenModal()} className="bg-[#1B2654] text-white px-10 py-5 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-[#A61A18] transition-all shadow-xl active:scale-95">+ Nuevo Registro</button>
                </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-6">
                        {products.filter(p => !p.ProductoPadre || p.ProductoPadre == 0).map(p => (
                            <ProductCard 
                                ...
                                subCount={products.filter(sub => String(sub.ProductoPadre) === String(p.IdProducto)).length}
                                onEdit={handleOpenModal}
                                onViewSubproducts={(parent) => { setSelectedParent(parent); setIsSidePanelOpen(true); }}
                            />
                        ))}
                    </div>
                )}
                                ...
                                <TransitionChild as={Fragment} enter="transform transition ease-in-out duration-500" enterFrom="translate-x-full" enterTo="translate-x-0" leave="transform transition ease-in-out duration-500" leaveFrom="translate-x-0" leaveTo="translate-x-full">
                                    <DialogPanel className="pointer-events-auto w-screen max-w-md">
                                        <div className="flex h-full flex-col bg-white shadow-2xl border-l-[12px] border-[#1B2654]">
                                            <div className="p-8 border-b border-slate-100 bg-slate-50/50">
                                                <div className="flex items-center justify-between mb-2">
                                                    <h2 className="text-2xl font-black text-[#1B2654] uppercase tracking-tighter">Variantes</h2>
                                                    <button onClick={() => setIsSidePanelOpen(false)} className="text-slate-400 hover:text-red-500 transition-colors">✕</button>
                                                </div>
                                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Base: {selectedParent?.Nombre}</p>
                                            </div>
                                            ...
                                            <div className="flex-1 overflow-y-auto p-8 space-y-3 custom-scrollbar">
                                                {products.filter(sub => String(sub.ProductoPadre) === String(selectedParent?.IdProducto)).map(sub => (
                                                    <div key={sub.IdProducto} className="group flex items-center justify-between p-5 rounded-[1.5rem] border-2 border-slate-100 hover:border-[#1B2654] hover:bg-slate-50 transition-all">
                                                        <div className="flex flex-col">
                                                            <span className="text-xs font-black text-slate-800 uppercase tracking-tight">{sub.Nombre}</span>
                                                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-1">{sub.UnidadMedida} | {sub.Clasificacion}</span>
                                                        </div>
                                                        <button onClick={() => { handleOpenModal(sub); setIsSidePanelOpen(false); }} className="p-3 bg-slate-100 text-slate-400 rounded-xl group-hover:bg-[#1B2654] group-hover:text-white transition-all shadow-sm">
                                                            <IconEdit />
                                                        </button>
                                                    </div>
                                                ))}
                                            ...
                                            <div className="p-8 bg-slate-50 border-t border-slate-100">
                                                <button onClick={() => { handleOpenModal({ ...initialFormState, EsSubproducto: "1", ProductoPadre: selectedParent.IdProducto }); setIsSidePanelOpen(false); }} className="w-full py-4 bg-[#A61A18] text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-lg hover:bg-red-700 transition-all">
                                                    + Añadir Variante
                                                </button>
                                            </div>
                                        </div>
            ...
            <Transition show={isDialogOpen} as={Fragment}>
                <Dialog onClose={() => !isSaving && setIsDialogOpen(false)} className="relative z-[200]">
                    ...
                    <div className="fixed inset-0 flex items-center justify-center p-4">
                        <TransitionChild as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                            <DialogPanel className="w-full max-w-md bg-white rounded-[3rem] p-8 shadow-2xl relative overflow-hidden border-b-[12px] border-[#1B2654]">
                                {isSaving && <div className="absolute inset-0 z-[60] flex items-center justify-center bg-white/70 backdrop-blur-sm"><LoadingDiv /></div>}
                                
                                <DialogTitle className="text-2xl font-black text-slate-800 uppercase tracking-tighter mb-8 text-center">
                                    {action === 'create' ? 'Nuevo Producto' : 'Actualizar Información'}
                                </DialogTitle>
                                ...
                                <form onSubmit={handleSave} className="space-y-5">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Unidad</label>
                                            <select value={formData.UnidadMedida} onChange={e => setFormData({ ...formData, UnidadMedida: e.target.value })} className="w-full px-4 py-4 rounded-2xl bg-slate-100 font-bold outline-none text-xs border-2 border-transparent focus:border-[#1B2654]">
                                                <option value="kg">KILOGRAMOS</option>
                                                <option value="Pieza">PIEZAS</option>
                                        ...
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Clasificacion</label>
                                            <select value={formData.Clasificacion} onChange={e => setFormData({ ...formData, Clasificacion: e.target.value })} className="w-full px-4 py-4 rounded-2xl bg-slate-100 font-bold outline-none text-xs border-2 border-transparent focus:border-[#1B2654]">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Ubicación en Almacén</label>
                                        <div className="bg-slate-50 p-2 rounded-[1.8rem] border-2 border-slate-100 space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
                                            {almacenes.map((alm) => {
                                                const isSelected = formData.AlmacenesSeleccionados.includes(alm.IdAlmacen);
                                                return (
                                                    <div 
                                                        key={alm.IdAlmacen} 
                                                        onClick={() => toggleAlmacen(alm.IdAlmacen)}
                                                        className={`flex items-center justify-between p-3.5 rounded-2xl cursor-pointer transition-all border-2 ${
                                                            isSelected 
                                                            ? 'bg-[#1B2654] border-[#1B2654] text-white shadow-lg translate-x-1' 
                                                            : 'bg-white border-transparent text-slate-600 hover:border-slate-200 shadow-sm'
                                                        }`}
                                                    >
                                                        <span className="text-[9px] font-black uppercase tracking-tight">{alm.Nombre}</span>
                                                        <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors ${isSelected ? 'bg-white border-white' : 'border-slate-200'}`}>
                                                            {isSelected && <div className="w-1.5 h-1.5 bg-[#1B2654] rounded-full" />}
                                                        </div>
                                                    </div>
                                                );
                                            })}
                                    ...
                                    {/* Vinculación */}
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Producto Padre</label>
                                        <select value={formData.ProductoPadre || ""} onChange={e => setFormData({ ...formData, ProductoPadre: e.target.value })} className="w-full px-5 py-4 rounded-2xl bg-slate-100 font-bold outline-none text-xs border-2 border-transparent focus:border-[#1B2654]">
                                            <option value="">NINGUNO (ESTABLECER COMO PADRE)</option>
                                            {products.filter(p => p.IdProducto !== formData.IdProducto && (!p.ProductoPadre || p.ProductoPadre == 0)).map(p => (
                                    ...
                                    <div className="flex gap-4 pt-4">
                                        <button type="button" onClick={() => setIsDialogOpen(false)} className="flex-1 py-4 text-slate-400 font-black text-xs uppercase hover:text-slate-600 transition-colors">Cancelar</button>
                                        <button type="submit" disabled={isSaving} className="flex-[2] py-4 bg-[#1B2654] text-white rounded-2xl font-black text-xs uppercase shadow-xl hover:bg-[#A61A18] transition-all">Guardar Producto</button>
                                    </div>
                                </form>
                            </DialogPanel>
                        </TransitionChild>
