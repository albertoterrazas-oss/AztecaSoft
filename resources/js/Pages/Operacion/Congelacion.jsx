...
import { Snowflake } from 'lucide-react'; // Opcional: requiere instalar lucide-react
const route = (name) => {
    const routeMap = {
...
export default function WeighingDashboard() {
    ...
    const fetchProductosLote = async (idLote) => {
        try {
            const res = await axios.post("/api/ProductosLotesHistorial", { opcion: 'A', Lote: idLote, idAlmacen: 5 });
    ...
    const registrarPesaje = async (brutoConfirmado, taraConfirmada) => {
        ...
        try {
            ...
            const payload = {
                ...
                piezas: piezas,
                id_area_entrada: 5, // ID fijo para Limpieza/Proceso
        ...
        return (
            <div className="min-h-screen bg-slate-100 p-8 flex flex-col items-center justify-center font-black uppercase">
                <div className="max-w-4xl w-full">
                    <h1 className="text-4xl text-center mb-10 italic font-black text-slate-800">Panel de Congelacion</h1>
                    <div className="grid gap-4">
                        {lotes.length > 0 ? (
                            ...
                            <div className="bg-white p-12 rounded-[2.5rem] shadow-xl border-4 border-dashed border-slate-300 flex flex-col items-center">
                                ...
                                <h2 className="text-2xl text-slate-400 text-center">No hay lotes de congelacion disponibles</h2>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-[2rem] border-4 border-slate-200 flex flex-col gap-3">
                    ...
                    <div>
                        <label className={`text-[10px] block mb-2 text-center tracking-widest font-black uppercase ${!selectedArea ? 'text-red-500 animate-pulse' : 'text-slate-400'}`}>
                            {!selectedArea ? "⚠️ Elija congelador" : "Almacen de Salida"}
                        </label>
                        ...
                        <div className="flex gap-1">
                            {almacenes.map(a => (
                                <button
                                    key={a.id || a.IdAlmacen}
                                    onClick={() => setSelectedArea(a.id || a.IdAlmacen)}
                                    className={`flex-1 py-3 rounded-xl text-[9px] transition-all border-b-4 font-black flex flex-col items-center justify-center gap-1 ${selectedArea === (a.id || a.IdAlmacen)
                                            ? 'bg-yellow-400 border-yellow-600 text-yellow-900'
                                            : 'bg-slate-100 border-slate-300 text-slate-400'
                                        }`}
                                >
                                    {/* ICONO: Puedes usar el componente de Lucide o un Emoji ❄️ */}
