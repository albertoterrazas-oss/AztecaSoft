...
export default function DeshueseDashboard() {
    ...
    const netWeightDashboard = useMemo(() => {
        const val = parseFloat(currentWeight || 0) - parseFloat(taraGlobal || 0);
        return Math.max(0, val).toFixed(2);
    }, [currentWeight, taraGlobal]);
    ...
    const registrarDeshuese = async (pesoBrutoFinal) => {
        setIsProcessing(true);
        const pesoNeto = Math.max(0, parseFloat(pesoBrutoFinal) - parseFloat(taraGlobal)).toFixed(2);
    ...
    if (!selectedLote) {
        return (
            <div className="min-h-screen bg-slate-100 p-8 flex flex-col items-center justify-center font-black uppercase">
                <div className="max-w-4xl w-full">
                    <h1 className="text-4xl text-center mb-10 italic font-black text-slate-800">Panel de Pesaje: DESHUESE</h1>
                    <div className="grid gap-4">
                        {lotes.length > 0 ? (
                            lotes.map((lote) => (
                                <button key={lote.Lote} onClick={() => handleLoteSelect(lote)} className="bg-white p-6 rounded-[2.5rem] flex items-center justify-between border-4 border-transparent hover:border-red-600 transition-all shadow-xl group">
                                    <div className="text-left font-black">
                                        <span className="text-xs text-red-600 uppercase font-black tracking-widest">LOTE #{lote.Lote}</span>
                                        <h3 className="text-2xl leading-none text-slate-700">{lote.Proveedor}</h3>
                                    </div>
                                    <div className="bg-slate-100 group-hover:bg-red-600 group-hover:text-white p-5 rounded-3xl transition-all">
                            ...
                            <div className="bg-white p-12 rounded-[2.5rem] shadow-xl border-4 border-dashed border-slate-300 flex flex-col items-center">
                                ...
                                <h2 className="text-2xl text-slate-400 text-center">No hay lotes activos</h2>
                            </div>
                        )}
                    </div>
                </div>
    ...
    return (
        <div className="relative flex flex-col lg:flex-row h-screen bg-slate-200 p-4 gap-4 font-black uppercase overflow-hidden">
            ...
            <BasculaModal
                ...
                currentReading={currentWeight}
                buttonText="FIJAR TARA"
                colorClass="bg-red-600 border-red-900"
                onSimulate={() => setCurrentWeight((Math.random() * 5 + 2).toFixed(2))}
                onConfirm={(peso) => { setTaraGlobal(peso); setCurrentWeight("0.00"); setShowTaraModal(false); }}
                onClose={() => { setSelectedLote(null); setShowTaraModal(false); }}
                showSimulate={true}
            />
            ...
            <BasculaModal
                ...
                tara={taraGlobal}
                buttonText="GUARDAR CORTE"
                colorClass="bg-emerald-600 border-emerald-900"
                onSimulate={() => setCurrentWeight((Math.random() * 40 + 10).toFixed(2))}
                onConfirm={registrarDeshuese}
                onClose={() => { setShowPesarModal(false); setCurrentWeight("0.00"); }}
                showSimulate={true}
            ...
            <div className="flex-1 flex flex-col min-h-0 gap-4">
                <header className="flex justify-between items-center bg-white p-6 rounded-[2.5rem] shadow-md border-b-[10px] border-slate-300">
                    <div>
                        <button onClick={() => { setSelectedLote(null); setParentFilter(null); }} className="text-[10px] text-slate-400 hover:text-red-600 mb-1 block">← CAMBIAR LOTE</button>
                        <h1 className="text-2xl text-slate-800 leading-none">{selectedLote.Proveedor}</h1>
                        <p className="text-[10px] text-blue-600 tracking-widest uppercase">LOTE: {selectedLote.Lote} | TARA: {taraGlobal} KG</p>
                    </div>
                    {selectedParent && (
                        <div className="bg-emerald-500 text-white px-6 py-3 rounded-2xl text-[10px] border-b-4 border-emerald-700">
                            ORIGEN: {selectedParent.Nombre}
                ...
                <div className="flex-1 overflow-y-auto custom-scroll pr-2">
                    <div className="mb-4 flex justify-between px-4">
                        <span className="text-xs text-slate-500 tracking-widest">{parentFilter ? "SUBPRODUCTOS" : "ELIJA PRODUCTO PADRE"}</span>
                        {parentFilter && <button onClick={() => { setParentFilter(null); setSelectedParent(null); setSelectedChild(null); }} className="text-red-600 text-xs underline">VOLVER</button>}
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {productosVisibles.map((p) => (
                            <button
            ...
            <aside className="w-full lg:w-[400px] flex flex-col gap-4">
                <div className="bg-slate-900 rounded-[3rem] p-8 border-b-[12px] border-black shadow-2xl">
                    <div className="bg-[#0f1713] rounded-[2rem] p-10 border-4 border-black text-center relative overflow-hidden">
                        <div className="text-8xl font-mono text-green-400">{netWeightDashboard}</div>
                        <span className="text-green-900 text-[10px] mt-4 block tracking-[0.3em] font-black italic">NETO EN BÁSCULA</span>
                    </div>
                </div>

                <div className="bg-white p-6 rounded-[3rem] border-b-[12px] border-slate-300 flex flex-col gap-4 shadow-xl">
                    <button onClick={() => { setCurrentWeight("0.00"); setShowTaraModal(true); }} className="bg-slate-800 text-white py-5 rounded-2xl text-xs border-b-8 border-black active:translate-y-1 active:border-b-0 font-black">
                        {parseFloat(taraGlobal) > 0 ? "RE-PESAR TARA" : "1. CONFIGURAR TARA"}
                    </button>

                    <div className="w-full p-2">
                        <label className="text-[10px] block mb-2 text-center font-black text-slate-400 uppercase tracking-widest">Piezas Producidas</label>
                        <div className="flex items-stretch gap-2 h-20">
                            <button type="button" onClick={() => setPiezas(prev => Math.max(0, (parseInt(prev) || 0) - 1).toString())} className="flex-none w-16 bg-slate-200 rounded-2xl border-b-8 border-slate-300 active:translate-y-1 active:border-b-0 text-3xl font-black">-</button>
                            <input type="number" value={piezas} onChange={(e) => setPiezas(e.target.value)} className="flex-1 bg-slate-100 border-b-8 border-slate-200 rounded-2xl font-black text-center text-4xl outline-none" placeholder="0" />
                            <button type="button" onClick={() => setPiezas(prev => ((parseInt(prev) || 0) + 1).toString())} className="flex-none w-16 bg-slate-200 rounded-2xl border-b-8 border-slate-300 active:translate-y-1 active:border-b-0 text-3xl font-black">+</button>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                        {almacenes.map(a => (
                            <button key={a.IdAlmacen} onClick={() => setAreaDestino(a.IdAlmacen)}
                                className={`py-4 rounded-2xl text-[10px] transition-all border-b-4 font-black ${areaDestino === a.IdAlmacen ? 'bg-yellow-400 border-yellow-600 text-yellow-900' : 'bg-slate-100 border-slate-300 text-slate-400'}`}>
                                DESTINO: {a.Nombre?.substring(0, 20)}
                            </button>
                        ))}
                    </div>
                    ...
                    <button
                        ...
                        className={`w-full py-8 rounded-[2.5rem] text-3xl border-b-[12px] shadow-2xl transition-all font-black
                            ${(!selectedChild || !areaDestino)
                                ...
                                : "bg-emerald-600 text-white border-emerald-800 hover:bg-emerald-500 active:translate-y-2 active:border-b-0"}`}
                    >
                        {!selectedParent ? "ELIJA PADRE" : !selectedChild ? "ELIJA CORTE" : "2. PESAR CORTE"}
                    </button>
                </div>
            </aside>
        </div>
