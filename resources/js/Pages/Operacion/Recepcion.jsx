...
function FinishSessionDialog({
    ...
    // Limpiar observaciones al abrir
    useEffect(() => {
        if (isOpen) setObs("");
    }, [isOpen]);
...
export default function WeighingDashboard() {
    ...
    useEffect(() => {
        const fetchCatalogos = async () => {
            try {
                ...
                // Ajuste según estructura de Laravel (usualmente res.data)
                const listaProveedores = resProv.data.data || resProv.data;
                const listaProductos = resProd.data.data || resProd.data;

                setDbProviders(listaProveedores);
                setDbProducts(listaProductos);

                if (listaProductos.length > 0) {
                    setSelectedProduct(listaProductos[0]);
                }
            } catch (error) {
                console.error(error);
    ...
    const totalKilos = records
        .reduce((acc, rec) => acc + parseFloat(rec.peso), 0)
        .toFixed(2);
    ...
    return (
        <div className="h-[90vh] bg-slate-50 p-4">
            {!isSessionActive ? (
                /* VISTA INICIAL: SELECCIÓN DE PROVEEDOR Y FOLIO */
                <div className="flex h-full items-center justify-center">
                    <div className="w-full max-w-md bg-white p-10 rounded-[2.5rem] shadow-xl border border-slate-100">
                        ...
                        <form onSubmit={handleStartSession} className="space-y-6">
                            ...
                            <button type="submit" className="w-full bg-red-600 text-white font-black py-5 rounded-2xl uppercase tracking-widest shadow-lg shadow-red-100 transition-all hover:bg-red-700">
                                Comenzar Pesaje
                            </button>
                        </form>
                    </div>
                           ...
                           <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                            <table className="w-full">
                                <thead className="bg-slate-50 border-b border-slate-100">
                                    <tr>
                                        <th className="p-4 text-[10px] font-black uppercase text-slate-400">Hora</th>
                                        <th className="p-4 text-[10px] font-black uppercase text-slate-400">Producto</th>
                                        <th className="p-4 text-right text-[10px] font-black uppercase text-slate-400">Peso Registrado</th>
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {records.map((reg) => (
                                        <tr key={reg.id} className="group hover:bg-slate-50/50 transition-colors">
                                            <td className="P-2 text-center font-mono text-slate-400 text-xs">{reg.hora}</td>
                                            <td className="P-2 text-center font-black text-xs uppercase text-slate-800">{reg.producto}</td>
                                            <td className="P-2 text-center text-right font-mono font-black text-red-600 text-xl">{reg.peso} <span className="text-[10px]">{reg.unidad}</span></td>
                                            <td className="P-2 text-center">
                                                <button onClick={() => setRecords(records.filter(r => r.id !== reg.id))} className="p-3 text-slate-200 hover:text-red-600 transition-colors">
                    ...
                    <aside className="w-full lg:w-80 flex flex-col gap-4">
                        <div className="bg-slate-900 rounded-[2.5rem] p-8 text-center text-white shadow-2xl">
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">
                                Peso Actual - {selectedProduct?.Nombre}
                            </p>
                        ...
                        <button
                            ...
                            className=" bg-slate-200 text-slate-600 font-black py-4 rounded-2xl uppercase text-[10px] tracking-widest hover:bg-slate-300"
                        >
                            Finalizar Lote
                        </button>
                    </aside>
